package cn.qiany.basic.module.infra.service.codegen.inner;
...
import cn.qiany.basic.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.qiany.basic.framework.common.util.date.LocalDateTimeUtils;
import cn.qiany.basic.framework.common.util.object.BeanUtils;
import cn.qiany.basic.framework.common.util.object.ObjectUtils;
import cn.qiany.basic.framework.common.util.string.StrUtils;
import cn.qiany.basic.framework.excel.core.annotations.DictFormat;
import cn.qiany.basic.framework.excel.core.convert.DictConvert;
...
import cn.qiany.basic.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.qiany.basic.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.qiany.basic.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.qiany.basic.module.infra.enums.codegen.CodegenSceneEnum;
import cn.qiany.basic.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.qiany.basic.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.qiany.basic.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 qybasic-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 qybasic-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.qiany.basic.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/qybasic-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/qybasic-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "qybasic-module-${table.moduleName}/" + // 顶级模块
                "qybasic-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "qybasic-module-${table.moduleName}/" + // 顶级模块
                "qybasic-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "qybasic-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "qybasic-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "qybasic-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "qybasic-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
